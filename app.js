...
function renderGraph(nodes, links, rootItem) {
  ...
  for (let i=0;i<depthsSorted.length;i++){
    ...
    const anchors = outputs.map(anchorRightPos);
    const topY = Math.min(...anchors.map(a=>a.y));
    const bottomY = Math.max(...anchors.map(a=>a.y));
    const spineX = anchors[0].x;

    spineSvg += `
      <line class="graph-spine-vertical"
        x1="${spineX}" y1="${bottomY}"
        x2="${spineX}" y2="${topY}"
        stroke="var(--spine-color)" stroke-width="2"
        marker-end="url(#arrow-up)" />
    `;

    if (i+1 < depthsSorted.length) {
      const nextDepth = depthsSorted[i+1];
      const nextCol = columns[nextDepth] || [];
      const inputs = nextCol.filter(n => n.hasInputAnchor && !n.raw);

      if (!inputs.length) continue;

      const inAnchors = inputs.map(anchorLeftPos);
      const inTopY = Math.min(...inAnchors.map(a=>a.y));
      const inBottomY = Math.max(...inAnchors.map(a=>a.y));
      const nextX = inAnchors[0].x;

      spineSvg += `
        <line class="graph-spine-horizontal"
          x1="${spineX}" y1="${topY}"
          x2="${nextX}" y2="${topY}"
          stroke="var(--spine-color)" stroke-width="2"
  ...
  for (const link of links || []) {
    ...
    if (dst.depth > minDepth+1) continue;

    inner += `
      <line class="graph-edge direct-node-line"
        x1="${src.x}" y1="${src.y}"
        x2="${dst.x}" y2="${dst.y}"
  ...
  /* ---------------- NODE + HELPERS (NO ARROWS) ---------------- */
  // ⬅ unchanged from your original implementation
  // helper dots, node-to-helper lines, labels, circles, numbers
  // intentionally omitted here for brevity in explanation
  // BUT in your actual file, keep this section exactly as-is
